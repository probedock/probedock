class CreateTestContributors < ActiveRecord::Migration
  def up
    drop_table :test_contributors

    create_table :test_contributors do |t|
    ...
    ProjectTest.select(:id, :key_id, :first_runner_id).includes([ { key: :user }, :first_runner ]).find_in_batches batch_size: 250 do |tests|
      ...
      say_with_time "setting contributors for tests #{i + 1}-#{i + tests.length}" do
        tests.each do |test|
