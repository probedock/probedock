...
describe ProcessApiTestRun do
  ...
  it "should enrich test data with project and version", rox: { key: '0c55f491f8e5' } do
    ...
    ProcessApiTest.should_receive(:new).ordered.with sample_payload[:r][0][:t][0].merge(additional_test_data), kind_of(TestRun), cache
    ProcessApiTest.should_receive(:new).ordered.with sample_payload[:r][0][:t][1].merge(additional_test_data), kind_of(TestRun), cache

    additional_test_data = sample_payload[:r][1].pick :j, :v
    ProcessApiTest.should_receive(:new).ordered.with sample_payload[:r][1][:t][0].merge(additional_test_data), kind_of(TestRun), cache
  ...
  it "should trigger an api:test_run event on the application", rox: { key: '06493acd8c6a' } do
    ROXCenter::Application.events.should_receive(:fire).with 'api:test_run', kind_of(ProcessApiTestRun)
    process_test_run
  end
  ...
  it "should correctly set the run's cached counters", rox: { key: '5fdac6019cb7' } do
    ...
    ProcessApiTest.stub(:new){ |*args| state = states.shift; test_stub state[:passed], state[:active] }
  ...
  def process_test_run data = sample_payload, runner = user, time_received = time_received, cache = cache
    ProcessApiTestRun.new data, runner, time_received, cache
  end
end
