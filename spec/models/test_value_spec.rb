...
describe TestValue, probe_dock: { tags: :unit } do
  ...
  context "validations" do
    ...
    it "should ensure that the contents are not longer than 65535 bytes", probe_dock: { key: 'dfa244369dd1' } do
      contents = 'x' * 65534
      contents << "\u3042"
      value = build :test_value, contents: contents
    ...
    context "with an existing value" do
      ...
      it(nil, probe_dock: { key: '3821045055d7' }){ should validate_uniqueness_of(:name).scoped_to(:test_info_id) }
    end
  end

  context "associations" do
    it(nil, probe_dock: { key: 'd43992d77922' }){ should belong_to(:test_info) }
  end
  ...
  context "database table" do
    ...
    it(nil, probe_dock: { key: '07774b12df6f' }){ should have_db_column(:test_info_id).of_type(:integer).with_options(null: false) }
    it(nil, probe_dock: { key: 'bb180a3fb1d4' }){ should have_db_index([ :test_info_id, :name ]).unique(true) }
  end
end
