...
describe Project do
  ...
  describe "when created" do
    ...
    it "should return the url token with #to_param", rox: { key: '1b9e112d286b' } do
      expect(subject.to_param).to eq(subject.url_token)
    end

    it "should automatically generate a metric key", rox: { key: 'fd6fd39c5a30' } do
  ...
  describe "validations" do
    ...
    it(nil, rox: { key: '67b690e62453' }){ should ensure_length_of(:url_token).is_at_most(25) }
    # TODO: find out why these allow_value specs make payload processing fail
    it("should allow alphanumeric characters, hyphens and underscores for url_token", rox: { key: 'f50b3a3da9e9' }){ should allow_value('a', 'a_b', 'a-b').for(:url_token) }
  ...
  describe "database table" do
    ...
    it(nil, rox: { key: 'acf557d6db43' }){ should have_db_column(:updated_at).of_type(:datetime).with_options(null: false) }
    it(nil, rox: { key: '6c8efd97b251' }){ should have_db_index(:api_id).unique(true) }
    it(nil, rox: { key: '36c6217578cd' }){ should have_db_index(:metric_key).unique(true) }
  end
end
