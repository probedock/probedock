...
describe StatusData do
  ...
  it "should return cached dates", rox: { key: 'ea29408c00a8' } do
    t1, t2, t3 = 1.day.ago.to_ms, 2.days.ago.to_ms, 3.days.ago.to_ms
  ...
  it "should set missing dates to now", rox: { key: 'd406a39215b8' } do
    ...
    expect(subject.compute).to include(lastApiPayload: 123, lastTestDeprecation: now.to_ms, lastTestCounters: now.to_ms)
  end
  ...
  describe "events" do
    ...
    it "should touch the last api payload time on the api:payload event", rox: { key: '9d1d4f8f598d' } do
      ...
      expect(subject.compute).to include(lastApiPayload: now.to_ms, lastTestDeprecation: last_test_deprecation, lastTestCounters: last_test_counters)
    end
    ...
    it "should touch the last test deprecation date on the test:deprecated event", rox: { key: '85811805ed31' } do
      ...
      expect(subject.compute).to include(lastApiPayload: last_api_payload, lastTestDeprecation: now.to_ms, lastTestCounters: last_test_counters)
    end
    ...
    it "should touch the last test counters date on the test:counters event", rox: { key: '92818bf733e5' } do
      ...
      expect(subject.compute).to include(lastApiPayload: last_api_payload, lastTestDeprecation: last_test_deprecation, lastTestCounters: now.to_ms)
    end
  end
end
