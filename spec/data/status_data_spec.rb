...
describe StatusData do
  ...
  it "should return cached dates", rox: { key: 'ea29408c00a8' } do
    ...
    expect(subject.compute).to eq(lastApiPayload: t1, lastTestDeprecation: t2, lastTestCounters: t3)
  end
  ...
  it "should set missing dates to now", rox: { key: 'd406a39215b8' } do
    ...
    expect(subject.compute).to eq(lastApiPayload: 123, lastTestDeprecation: to_timestamp(now), lastTestCounters: to_timestamp(now))
  end
  ...
  describe "events" do
    ...
    it "should touch the last api payload time on the api:payload event", rox: { key: '9d1d4f8f598d' } do
      ...
      expect(subject.compute).to eq(lastApiPayload: to_timestamp(now), lastTestDeprecation: last_test_deprecation, lastTestCounters: last_test_counters)
    end
    ...
    it "should touch the last test deprecation date on the test:deprecated event", rox: { key: '85811805ed31' } do
      ...
      expect(subject.compute).to eq(lastApiPayload: last_api_payload, lastTestDeprecation: to_timestamp(now), lastTestCounters: last_test_counters)
    end
    ...
    it "should touch the last test counters date on the test:counters event", rox: { key: '92818bf733e5' } do
      ...
      expect(subject.compute).to eq(lastApiPayload: last_api_payload, lastTestDeprecation: last_test_deprecation, lastTestCounters: to_timestamp(now))
    end
  end
