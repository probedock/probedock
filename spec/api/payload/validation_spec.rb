...
describe "API payload validations", probe_dock: { tags: :unit } do
  ...
  it "should accept a valid payload", probe_dock: { key: 'b0554b845238' } do
    assert_accepted sample_payload
  end

  context "for existing tests" do

    it "should accept an existing test without a name", probe_dock: { key: '59cf76bea3e1' } do
    ...
    it "should accept a test with a null category", probe_dock: { key: '4463af7bb73a' } do
      sample_payload[:r][0][:t][0][:c] = nil
      assert_accepted sample_payload
    end

    it "should accept a test with empty tags", probe_dock: { key: '2a155ed13d54' } do
      sample_payload[:r][1][:t][0][:g] = []
      assert_accepted sample_payload
    end

    it "should accept a test with empty tickets", probe_dock: { key: '4c17a4e712f6' } do
      sample_payload[:r][1][:t][0][:t] = []
      assert_accepted sample_payload
    end
  end

  it "should fail if the body is bigger than 16777215 bytes", probe_dock: { key: '4576a2aae76d' } do
    assert_fail '{"foo":"' + ("x" * 16777215) + '"}', :payloadTooBig, false
  end

  it "should fail if the JSON is invalid", probe_dock: { key: '58338a3e18b7' } do
    assert_fail 'fubar', :invalidJson, false
  end

  it "should fail if the payload is not an object", probe_dock: { key: '31ae39bfd026' } do
    assert_fail [].to_json, :invalidValue, ''
  end

  it "should fail if the duration of the test run is missing", probe_dock: { key: 'cc07998847f1' } do
    sample_payload.delete :d
    assert_fail sample_payload, :missingKey, "/d"
  end

  it "should fail if the duration of the test run is invalid", probe_dock: { key: 'ed475fda4914' } do
    sample_payload[:d] = 'fubar'
    assert_fail sample_payload, :invalidValue, "/d", got('fubar')
  end

  it "should fail if the uid of the test run is not a string", probe_dock: { key: '056168b73b33' } do
    sample_payload[:u] = 666
    assert_fail sample_payload, :invalidValue, "/u", got(:number)
  end

  it "should fail if the uid of the test run is too long", probe_dock: { key: 'b28146cfff22' } do
    sample_payload[:u] = 'x' * 256
    assert_fail sample_payload, :valueTooLong, "/u", got('256'.to_sym)
  end

  it "should fail if the uid of the test run is blank", probe_dock: { key: '9f48ae42f306' } do
    sample_payload[:u] = '   '
    assert_fail sample_payload, :blankValue, "/u"
  end

  it "should fail if the test run corresponding to the uid was created by another user", probe_dock: { key: '615c01e73ae5' } do
  ...
  it "should fail if the group of a test run is not a string", probe_dock: { key: '729addd2d604' } do
    sample_payload[:g] = 789
    assert_fail sample_payload, :invalidValue, "/g", got(:number)
  end

  it "should fail if the group of a test run is too long", probe_dock: { key: 'a91c4cf82424' } do
    sample_payload[:g] = 'x' * 256
    assert_fail sample_payload, :valueTooLong, "/g", got('256'.to_sym)
  end

  it "should fail if the group of a test run is blank", probe_dock: { key: '6bfb6b02d77e' } do
    sample_payload[:g] = '  '
    assert_fail sample_payload, :blankValue, "/g"
  end

  it "should fail if the test run has no results", probe_dock: { key: '02702eaaced6' } do
    assert_fail sample_payload.omit(:r), :missingKey, "/r"
  end

  it "should fail if the test run contains zero results", probe_dock: { key: '343c623ec6fd' } do
    assert_fail sample_payload.merge(r: []), :emptyArray, "/r"
  end

  it "should fail if the results of the test run are not an array", probe_dock: { key: '71e0154abd26' } do
    assert_fail sample_payload.merge(r: { a: :b }), :invalidValue, "/r", got(:object)
  end

  it "should fail if the project of results is missing", probe_dock: { key: '5bc7ca8dfd0b' } do
    sample_payload[:r][0].delete :j
    assert_fail sample_payload, :missingKey, "/r/0/j"
  end

  it "should fail if the project of results is not a string", probe_dock: { key: '7039b314051e' } do
    sample_payload[:r][1][:j] = 123
    assert_fail sample_payload, :invalidValue, "/r/1/j", got(:number)
  end

  it "should fail if the project of results is blank", probe_dock: { key: 'a788052400f0' } do
    sample_payload[:r][0][:j] = '   '
    assert_fail sample_payload, :blankValue, "/r/0/j"
  end

  it "should fail if the project of results is too long", probe_dock: { key: '0678b771e15a' } do
    sample_payload[:r][0][:j] = 'x' * 256
    assert_fail sample_payload, :valueTooLong, "/r/0/j", got('256'.to_sym)
  end

  it "should fail if two results objects have the same project", probe_dock: { key: '854bd4c0e187' } do
    sample_payload[:r][1][:j] = sample_payload[:r][0][:j]
    assert_fail sample_payload, :duplicateProject, "/r/1/j", got(sample_payload[:r][1][:j])
  end

  it "should fail if the version of results is missing", probe_dock: { key: 'b255ab10a298' } do
    sample_payload[:r][0].delete :v
    assert_fail sample_payload, :missingKey, "/r/0/v"
  end

  it "should fail if the version of results is not a string", probe_dock: { key: '9ab28d2c4be1' } do
    sample_payload[:r][0][:v] = 456
    assert_fail sample_payload, :invalidValue, "/r/0/v", got(:number)
  end

  it "should fail if the version of results is blank", probe_dock: { key: '3e561aee5624' } do
    sample_payload[:r][1][:v] = '   '
    assert_fail sample_payload, :blankValue, "/r/1/v"
  end

  it "should fail if the version of results is too long", probe_dock: { key: 'c1ea79dd3bb7' } do
    sample_payload[:r][1][:v] = 'x' * 256
    assert_fail sample_payload, :valueTooLong, "/r/1/v", got('256'.to_sym)
  end

  it "should fail if test results are missing", probe_dock: { key: 'faac14a8286a' } do
    sample_payload[:r][0].delete :t
    assert_fail sample_payload, :missingKey, "/r/0/t"
  end

  it "should fail if test results are not an array", probe_dock: { key: '310f38293c79' } do
    sample_payload[:r][0][:t] = { a: :b }
    assert_fail sample_payload, :invalidValue, "/r/0/t", got(:object)
  end

  it "should fail if a test is not an object", probe_dock: { key: 'deb8b5da561a' } do
    sample_payload[:r][0][:t][0] = 'fubar'
    assert_fail sample_payload, :invalidValue, "/r/0/t/0", got(:string)
  end

  it "should fail if a test has no key", probe_dock: { key: '107ad717bbbb' } do
    sample_payload[:r][0][:t][0].delete :k
    assert_fail sample_payload, :missingKey, "/r/0/t/0/k", got(sample_payload[:r][0][:t][0][:n][0, 30])
  end

  it "should fail if the key of a test is invalid", probe_dock: { key: 'd84d7fa1c89f' } do
    sample_payload[:r][1][:t][0][:k] = 'fubar'
    assert_fail sample_payload, :invalidValue, "/r/1/t/0/k", got('fubar')
  end

  it "should fail if two tests have the same key in the same project", probe_dock: { key: '2b74ac95de1f' } do
  ...
  it "should fail if a test has the same key as a previously stored test result in the same run (by UID)", probe_dock: { key: '5667ad1f4c51' } do
  ...
  it "should fail if a test has an unknown key", probe_dock: { key: 'fb21e10441ce' } do
    sample_payload[:r][1][:t][0][:k] = 'x' * 12
    assert_fail sample_payload, :unknownTestKey, "/r/1/t/0/k", got('x' * 12), got(sample_payload[:r][1][:j])
  end

  it "should fail if a test has a key from the wrong project", probe_dock: { key: '2b6f47717974' } do
  ...
  it "should fail if test tags are not an array", probe_dock: { key: 'd42f4272fbe7' } do
    sample_payload[:r][0][:t][1][:g] = 'fubar'
    assert_fail sample_payload, :invalidValue, "/r/0/t/1/g", got(:string), got(sample_payload[:r][0][:t][1][:k])
  end

  it "should fail if a test tag is not a string", probe_dock: { key: '0a001ac4d4b3' } do
  ...
  it "should fail if a test tag is invalid", probe_dock: { key: 'bbd4ce17088a' } do
  ...
  it "should fail if a test tag is blank", probe_dock: { key: '0d3a5b2d4084' } do
    sample_payload[:r][0][:t][0][:g] = [ 'unit', 'integration', '   ' ]
    assert_fail sample_payload, :blankValue, "/r/0/t/0/g/2", got(sample_payload[:r][0][:t][0][:k])
  end

  it "should fail if a test tag is too long", probe_dock: { key: '5ebfe8b3e006' } do
  ...
  it "should not fail if a test has duplicate tags", probe_dock: { key: '50ad48970f5a' } do
    sample_payload[:r][0][:t][1][:g] = [ 'unit', 'integration', 'unit' ]
    assert_accepted sample_payload
  end

  it "should not fail if a test has duplicate case-insensitive tags", probe_dock: { key: 'fe3d5de7cc9b' } do
    sample_payload[:r][0][:t][0][:g] = [ 'unit', 'integration', 'Unit' ]
    assert_accepted sample_payload
  end

  it "should fail if test tickets are not an array", probe_dock: { key: '653e232ebde0' } do
    sample_payload[:r][0][:t][0][:t] = 'fubar'
    assert_fail sample_payload, :invalidValue, "/r/0/t/0/t", got(:string), got(sample_payload[:r][0][:t][0][:k])
  end

  it "should fail if a test ticket is not a string", probe_dock: { key: 'e80fe99629bb' } do
  ...
  it "should fail if a test ticket is blank", probe_dock: { key: '3a7860e955e4' } do
    sample_payload[:r][0][:t][0][:t] = [ 'JIRA-42', '   ', 'JIRA-1337' ]
    assert_fail sample_payload, :blankValue, "/r/0/t/0/t/1", got(sample_payload[:r][0][:t][0][:k])
  end

  it "should fail if a test ticket is too long", probe_dock: { key: '1044b0eb1b79' } do
  ...
  it "should fail if a test message is blank", probe_dock: { key: '4b8de51a2e36' } do
    sample_payload[:r][0][:t][1][:m] = '   '
    assert_fail sample_payload, :blankValue, "/r/0/t/1/m", got(sample_payload[:r][0][:t][1][:k])
  end

  it "should fail if a test has a message with more than 65535 one-byte characters", probe_dock: { key: '64c807c870db' } do
  ...
  it "should fail if a test has a message with more than 32767 two-byte characters", probe_dock: { key: 'e28b0060e786' } do
  ...
  it "should fail if a test has a message with more than 21845 three-byte characters", probe_dock: { key: 'a7f7dc2aed2f' } do
  ...
  it "should fail if a test data value is not a string", probe_dock: { key: '694fa7ad8603' } do
  ...
  it "should fail if a test data value is too long", probe_dock: { key: '8ce3929ed1cc' } do
  ...
  it "should fail if a test data value is too long in bytes", probe_dock: { key: '8545acfd99f7' } do
