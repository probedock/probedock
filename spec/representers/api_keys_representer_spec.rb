...
describe ApiKeysRepresenter, rox: { tags: :unit } do
  ...
  it(nil, rox: { key: 'b663d63e7ae9' }){ should have_curie(name: 'v1', templated: true, href: "#{uri(:doc_api_relation, name: 'v1')}:apiKeys:{rel}") }
  it(nil, rox: { key: '833b8ec53b7b' }){ should have_embedded('v1:api-keys', api_keys.collect{ |k| ApiKeyRepresenter.new(k).serializable_hash }) }
end
