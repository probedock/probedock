...
describe ApiKeyRepresenter, rox: { tags: :unit } do
  ...
  it(nil, rox: { key: '168bde141c89' }){ should have_only_properties(id: api_key.identifier, active: api_key.active, usageCount: api_key.usage_count, createdAt: api_key.created_at.to_i * 1000) }

  context "when the key has been used" do
    let(:api_key){ super().tap{ |k| k.usage_count = 42; k.last_used_at = Time.now; k.save! } }
    it(nil, rox: { key: '2f689a9f8e7a' }){ should have_properties(usageCount: 42, lastUsedAt: api_key.last_used_at.to_i * 1000) }
  end
